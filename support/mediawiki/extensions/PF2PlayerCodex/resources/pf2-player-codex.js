( function () {
	...
	function factionsPage( data, root ) { root.appendChild( heading( 'Factions' ) ); if ( data.canEdit ) { var create = document.createElement( 'button' ); create.textContent = 'Créer une faction'; create.onclick = function () { var name = window.prompt( 'Nom de la faction' ); if ( name ) { request( 'createFaction', '', '', { name: name } ).then( function ( response ) { var faction = response.pf2; return api().postWithToken( 'csrf', { action: 'edit', title: faction.wikiPageTitle, text: '', createonly: 1, summary: 'Création du carnet joueur PF2', format: 'json' } ).then( function () { window.location.assign( mw.util.getUrl( faction.wikiPageTitle ) ); } ); } ).catch( function ( error ) { mw.notify( error && error.error && error.error.info ? error.error.info : 'Création impossible', { type: 'error' } ); } ); } }; root.appendChild( create ); } var list = document.createElement( 'ul' ); data.factions.forEach( function ( f ) { var item = document.createElement( 'li' ); item.appendChild( link( f.wikiPageTitle, f.name ) ); list.appendChild( item ); } ); root.appendChild( list ); }
	function factionPage( data, root, faction ) { root.appendChild( heading( 'Organisation connue' ) ); var text = document.createElement( 'p' ); text.textContent = faction.parentFactionId ? 'Sous-faction de : ' + ( data.factions.filter( function ( f ) { return f.id === faction.parentFactionId; } )[0] || {} ).name : 'Aucune faction parente'; root.appendChild( text ); if ( data.canEdit ) { var select = document.createElement( 'select' ); var none = document.createElement( 'option' ); none.value = ''; none.textContent = 'Aucune faction parente'; select.appendChild( none ); data.factions.filter( function ( f ) { return f.id !== faction.id; } ).forEach( function ( f ) { var option = document.createElement( 'option' ); option.value = f.id; option.textContent = f.name; option.selected = f.id === faction.parentFactionId; select.appendChild( option ); } ); var save = document.createElement( 'button' ); save.textContent = 'Enregistrer le parent'; save.onclick = function () { request( 'updateFaction', faction.id, '', { parentFactionId: select.value || null } ).then( function () { window.location.reload(); } ).catch( function ( error ) { mw.notify( error && error.error && error.error.info ? error.error.info : 'Hiérarchie invalide', { type: 'error' } ); } ); }; root.appendChild( select ); root.appendChild( save ); } }
	$( function () { var page = pageTitle(); var content = document.querySelector( '#mw-content-text' ); if ( !content || ( page !== 'Personnages' && page !== 'Factions' && page.indexOf( 'Personnage:' ) !== 0 && page.indexOf( 'Faction:' ) !== 0 ) ) { return; } var root = document.createElement( 'section' ); root.id = 'pf2-player-codex'; content.insertBefore( root, content.firstChild ); api().get( { action: 'pf2playercodex', format: 'json' } ).then( function ( response ) { var data = response.pf2playercodex; if ( page === 'Personnages' ) { listPage( data, root ); } else if ( page === 'Factions' ) { factionsPage( data, root ); } else if ( page.indexOf( 'Personnage:' ) === 0 ) { var character = data.characters.filter( function ( item ) { return item.wikiPageTitle === page; } )[0]; if ( character ) { characterPage( data, root, character ); } } else { var faction = data.factions.filter( function ( item ) { return item.wikiPageTitle === page; } )[0]; if ( faction ) { factionPage( data, root, faction ); } } } ); } );
}() );

...
class ApiPF2Journals extends ApiBase {
    public function execute() {
        $config = MediaWikiServices::getInstance()->getMainConfig(); $base = rtrim( $config->get( 'PF2JournalsApiBase' ), '/' ); $number = $this->getRequest()->getInt( 'number', 0 );
        $canAdmin = $this->getUser()->isRegistered() && $this->getUser()->isAllowed( 'delete' );
        $headers = [];
        if ( $number > 0 && $canAdmin && (string)$config->get( 'PF2JournalsInternalKey' ) !== '' ) { $headers['X-PF2-Journals-Key'] = (string)$config->get( 'PF2JournalsInternalKey' ); }
        $url = $number > 0 ? $base . '/journals/' . rawurlencode( (string)$number ) : $base . '/journals';

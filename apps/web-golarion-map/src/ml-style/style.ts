import { FillLayerSpecification, LayerSpecification, LineLayerSpecification, SymbolLayerSpecification, StyleSpecification, ExpressionFilterSpecification, ExpressionSpecification } from "maplibre-gl";
...
export default function(HOST:string, BUILD_DATA_HASH: number, placeNamesFr: Record<string, string> = {}) {
  ...
  //single source of truth for FR labels; falls back to the raw vector-tile label when no translation exists
  const placeNameEntries = Object.entries(placeNamesFr);
  const labelExpr: ExpressionSpecification = placeNameEntries.length === 0
    ? (['get', Prop.label] as unknown as ExpressionSpecification)
    : ([
        'match',
        ['get', Prop.label],
        ...placeNameEntries.flatMap(([en, fr]) => [en, fr]),
        ['get', Prop.label]

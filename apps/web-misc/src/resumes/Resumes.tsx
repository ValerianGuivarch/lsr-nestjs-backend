...
type Actor = { uuid: string; name: string }
...
export default function Resumes() {
  ...
  const names = useMemo(() => new Map(actors.map((actor) => [actor.uuid, actor.name])), [actors])
  const nextNumber = useMemo(() => Math.max(0, ...resumes.map((resume) => resume.sessionNumber)) + 1, [resumes])
  const loadActors = async () => {
    try {
      const actorResponse = await fetch(`${endpoint}/actors`)
    ...
    } catch { setNotice('Les résumés sont disponibles ; Foundry est hors ligne, les noms de PJ ne peuvent pas être chargés.') }
  }
  const load = async () => {
    try {
      const resumeResponse = await fetch(`${endpoint}/sessions`)

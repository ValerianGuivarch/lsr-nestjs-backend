...
export type GeographySnapshot={schemaVersion?:number;regions:RegionRecord[];lieux:LieuRecord[];aliases?:Record<string,string>;parents?:Record<string,string>}
...
let lieux:LieuRecord[]=[]
let overrides:Overrides={aliases:{},parents:{}}
let nodes=new Map<string,GeographyNode>(), aliases=new Map<string,string>(), ambiguous=new Set<string>()
...
function rebuildGeography(){
  nodes=new Map();aliases=new Map();ambiguous=new Set()
...
export function applyGeographySnapshot(value:unknown):void{
  const raw=value&&typeof value==='object'&&!Array.isArray(value)?value as GeographySnapshot:{regions:[],lieux:[]}
  regions=Array.isArray(raw.regions)?raw.regions:[]
  lieux=Array.isArray(raw.lieux)?raw.lieux:[]
  overrides={aliases:raw.aliases??{},parents:raw.parents??{}}
  rebuildGeography()
}

export async function loadGeographyFromApi():Promise<void>{
  const response=await fetch('/apil7r/pf2-mj/geography',{cache:'no-store'})
  if(!response.ok)throw new Error(`Impossible de charger la géographie PF2 (HTTP ${response.status}).`)
  applyGeographySnapshot(await response.json())
}
...
export function expandedPlaceLabels(raw:string){const k=resolveGeographyKey(raw),n=nodes.get(k);if(!n)return[raw];return [n.label,...n.aliases,...geographyAncestors(k).flatMap(a=>[a.label,...a.aliases])]}
export function placeDisplay(raw:string){const k=resolveGeographyKey(raw),n=nodes.get(k);if(!n)return raw;const a=geographyAncestors(k)[0];return a&&normalizePlace(a.label)!==normalizePlace(n.label)?`${n.label} · ${a.label}`:n.label}
export function placeFilterOptions(raws:string[]):PlaceFilterOption[]{const keys=new Set<string>();for(const raw of raws){const k=resolveGeographyKey(raw);keys.add(k);for(const a of geographyAncestors(k))keys.add(a.key)}return [...keys].flatMap(k=>{const n=nodes.get(k);return n?[{value:k,label:n.label,depth:geographyAncestors(k).length,kind:n.kind}]:[]}).sort((a,b)=>a.label.localeCompare(b.label,'fr',{sensitivity:'base'}))}

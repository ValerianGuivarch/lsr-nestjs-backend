...
export class DiscordService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Mapping stable Foundry Actor UUID -> Discord user ID.
   *
   * Si un acteur n'est pas présent ici :
   * - son nom Foundry est quand même nettoyé ;
   * - aucune mention Discord n'est ajoutée.
   */
  private readonly actorDiscordIds: Record<string, string> = {
    'Actor.u5TLgUAsPlLH2lvJ': '448500183186145291', // Tsuyi
  }
  ...
  private async findResumeMessage(
    ...
    const matches = (
      ...
      Boolean(message && this.isResumeMessage(message, resume.id))
    ...
    for (;;) {
      ...
      const found = page.find((message) =>
        this.isResumeMessage(message, resume.id),
      )
  ...
  private isResumeMessage(
    ...
    return (
      ...
      message.content.includes(this.resumeSignature(resumeId))
    )
  }
  ...
  private async resumeMessagePayload(
    ...
    const names = new Map(
      (await this.foundry.listActors()).map((actor) => [
        actor.uuid,
        actor.name,
      ]),
    )
    ...
    const label = (uuid: string) =>
      this.actorLabel(uuid, names)

    const participants = resume.participants
      .map(label)
      .join(', ')
    ...
    const facts = [
      ...
      resume.sessionXp > 0 &&
        `XP total : ${
          resume.sessionXp * resume.participants.length
    ...
    const actorMentionIds = [
      ...
      .map((uuid) => this.actorDiscordIds[uuid])
    ...
    return {
      content: [
        ...
        facts || 'Aucune information complémentaire.',

        `-# ${this.resumeSignature(resume.id)}`,
      ]
        .filter(Boolean)
        .join('\n\n'),

      allowedMentions: {
        users: [...new Set(allowedUserIds)],
      },
    }
  }

  /**
   * Enlève une éventuelle indication de joueur à la fin du nom Foundry.
   *
   * Exemples :
   * "Tsuyi (Guilhem)" -> "Tsuyi"
   * "Pépin (Eric)"    -> "Pépin"
   * "Janira Gavix"    -> "Janira Gavix"
   */
  private actorDisplayName(name: string): string {
    return name
      .replace(/\s*\([^)]*\)\s*$/, '')
      .trim()
  }
  ...
  private actorLabel(
    ...
    const rawName = names.get(uuid) ?? uuid

    const name = this.actorDisplayName(rawName)

    const discordId = this.actorDiscordIds[uuid]
  ...
  private async findMentionedUserId(
    ...
    const member = members.find(
      (candidate) =>
        candidate.user.username.toLowerCase() === expected ||
        candidate.user.globalName?.toLowerCase() === expected,
    )
  ...
  private resumeSignature(id: string): string {
    return `pf2-resume:${id}`
  }

  private requireClient(): Client {
    if (!this.client) {
      throw new Error('Client Discord indisponible.')
    }
  ...
  private config(): DiscordConfig | null {
    ...
    if (token && clientId && guildId) {
      return {
        ...
        guildId,
        summaryChannelName:
          process.env['DISCORD_SUMMARIES_CHANNEL_NAME']?.trim() ||
          'test',
        mentionUsername:
          process.env['DISCORD_SUMMARIES_MENTION_USERNAME']?.trim() ||
          'valerian0276',
      }
    }
